...
Designs, decks, documents, socials, prints, mailings, and campaigns are seven pipelines behind one workflow: separate types, stores, routes, renderers, prompts, and editors, with the shared helpers (history, provenance, CSS scoping, fonts, Chrome, the fix-round loop, the model client) used by all. See `CLAUDE.md` for the rules.
...
deck, a document, a social post, a print piece, an email, or a display
ad campaign. The agent runs on your own model account.
...
editor, and the campaign editor have the same pair as **Read** (the
default) and **Edit**: Read shows the slide, the page, the frame, the
sheet, the email, or the ad as a reader sees it, with no selection
outlines. The document editor's
...
`GET /schemas/{design,deck,document,social,print,mailing,campaign,question-set}`. A demo session
...
`PUT /campaigns/{session}-candidate-N`. The run environment carries
