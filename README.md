...
| `campaign` | A display ad or a set of A/B variants: a product launch, a sale, brand awareness, an event, retargeting, an app install | a campaign with `size` and `ads` | the IAB unit: medium rectangle (300×250) by default, leaderboard (728×90), half page (300×600), skyscraper (160×600), or mobile banner (320×100) | `/campaigns/{id}` |
| `artwork` | A piece of cover art or a set of A/B variants: a video thumbnail, a channel banner, a profile header, a podcast or album cover, a book cover | an artwork with `size` and `covers` | the cover unit: thumbnail (1280×720) by default, channel banner (2560×1440), profile header (1500×500), album cover (3000×3000), or book cover (1600×2560) | `/artworks/{id}` |

Every kind shares the theme, the HTML and CSS rules, the workflow, the templates, and the uploads. Decks add a presenter view, an audience window that follows it, and a PPTX export. The deck JSON, routes, presenter, and PPTX come from Swift Deck, which is now part of this project. Documents add a PDF export, one sheet per page, and a DOCX export: a flowing Word file built from the page HTML, with the theme's fonts and colors as its styles. Socials add a PDF export, one sheet per frame (the file a LinkedIn carousel takes), and a zip of one PNG per frame (the files an Instagram carousel takes). Prints add a PDF export, one PDF page per sheet (the file a print shop takes), and a zip of one PNG per sheet. Mailings add a PDF export, one PDF page per email, a zip of one PNG per email, and a zip of one email-client HTML file per email. Campaigns add a PDF export, one PDF page per ad, and a zip of one PNG per ad: the files an ad platform takes. Artworks add a PDF export, one PDF page per cover, and a zip of one PNG per cover: the files the platforms take.
...
Designs, decks, documents, socials, prints, mailings, campaigns, and artworks are eight pipelines behind one workflow: separate types, stores, routes, renderers, prompts, and editors, with the shared helpers (history, provenance, CSS scoping, fonts, Chrome, the fix-round loop, the model client) used by all. See `CLAUDE.md` for the rules.
...
deck, a document, a social post, a print piece, an email, a display
ad campaign, or cover art. The agent runs on your own model account.
...
document editor, the social editor, the print editor, the mailing
editor, the campaign editor, and the artwork editor have the same pair
as **Read** (the default) and **Edit**: Read shows the slide, the page,
the frame, the sheet, the email, the ad, or the cover as a reader sees
it, with no selection outlines. The document editor's
...
standard, and long; the campaign editor's switches the size between
the five IAB units; the artwork editor's switches the size between the
five cover units.
...
For a campaign, the editor adds **PDF** (one PDF page per ad, through
Chrome) and **PNG** (a zip with one PNG per ad, through Chrome: the
files an ad platform takes) next to the HTML export. The ad notes hold
the click-through URL and the alt text, as a `Link:` line and an
`Alt:` line.

For an artwork, the editor adds **PDF** (one PDF page per cover,
through Chrome) and **PNG** (a zip with one PNG per cover, through
Chrome: the files the platforms take) next to the HTML export. The
cover notes hold the title context and the alt text, as a `Title:`
line and an `Alt:` line.

## Agent routes

External agents read `GET /instructions` and the schemas at
`GET /schemas/{design,deck,document,social,print,mailing,campaign,artwork,question-set}`. A demo session
...
`PUT /mailings/{session}-candidate-N`; a campaign session writes to
`PUT /campaigns/{session}-candidate-N`; an artwork session writes to
`PUT /artworks/{session}-candidate-N`. The run environment carries
...
Campaign-only routes: `GET /campaigns/{id}/ads/{n}.png`,
`GET /campaigns/{id}/export.pdf`, `GET /campaigns/{id}/export.zip`.
