...
/// The print toolbar's export group: the HTML file, the Chrome-backed
/// PDF for the print shop, and the Chrome-backed zip of one PNG per
/// sheet.
#[component]
fn PrintExportGroup(print_id: String, can_export_with_chrome: bool) -> Element {
    rsx! {
        div { class: "export-group",
            a {
                class: "button",
                href: "/prints/{print_id}/export",
                title: "Export as one HTML file",
                span { dangerous_inner_html: icons::DOWNLOAD }
                "HTML"
            }
            ChromeExportLink {
                href: format!("/prints/{print_id}/export.pdf"),
                label: "PDF",
                title: "Export as a PDF for the print shop, one page per sheet",
                is_enabled: can_export_with_chrome,
            }
            ChromeExportLink {
                href: format!("/prints/{print_id}/export.zip"),
                label: "PNG",
