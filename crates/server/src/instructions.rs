...
//! English.
//!
//! There are five artifact kinds. A demo session writes designs
...
use design_model::{Deck, Design, Document, Print, QUESTIONS_PER_TURN_LIMIT, Social};
...
/// Chart rules for data screens, slides, pages, frames, and sheets.
...
fn steps() -> Vec<String> {
    vec![
        "GET /schemas/design, GET /schemas/deck, GET /schemas/document, GET /schemas/social, GET /schemas/print, and GET /schemas/question-set. Read the JSON Schemas.".to_owned(),
        "GET /sessions/{id}. Read the request, the artifact_kind, the options, the state, the messages, the question sets, and the answers. The artifact_kind is `demo`, `deck`, `document`, `social`, or `print`. A demo session writes designs. A deck session writes decks. A document session writes documents. A social session writes socials. A print session writes prints. There is no brief: the request and the answers are the input.".to_owned(),
        ...
        "When the design, the deck, the document, the social, or the print is written, POST /sessions/{id}/complete, or exit with code 0. The session moves to reviewing.".to_owned(),
...
fn instructions() -> serde_json::Value {
    ...
    let example_print: serde_json::Value =
        serde_json::from_str(include_str!("../../../fixtures/sample-print.json"))
            .unwrap_or_default();
    serde_json::json!({
        "purpose": "Turn a request into HTML designs, decks, documents, social posts, or print pieces: ask a few questions in the chat, write candidates, then edit them from the chat. Swift Design keeps the workflow state, validates, renders, and lets the user edit. Swift Design makes no LLM API calls.",
        "session": "The run works on one session. Its id is in the SWIFT_DESIGN_SESSION_ID environment variable. The mode is in SWIFT_DESIGN_RUN_MODE: generation. The artifact kind is in SWIFT_DESIGN_ARTIFACT_KIND: demo, deck, document, social, or print. GET /sessions/{id} returns the state, the artifact_kind, the options, the question sets, the answers, and the messages.",
        "kinds": {
        ...
        "conventions": {
            ...
            "node_reference": "[screen N, node a/b/c <tag.class>: text] for a design; [slide N, node a/b/c <tag.class>: text] for a deck; [page N, node a/b/c <tag.class>: text] for a document; [frame N, node a/b/c <tag.class>: text] for a social; [sheet N, node a/b/c <tag.class>: text] for a print",
...
mod tests {
    ...
    use crate::instructions::{
        CHART_EXAMPLE, DECK_RULES, DEMO_RULES, DOCUMENT_RULES, PRINT_RULES, SOCIAL_RULES,
        instructions,
    };
    ...
    fn instructions_carry_social_rules_routes_and_the_example() {
        ...
        assert!(text.contains("demo, deck, document, social, or print"));
