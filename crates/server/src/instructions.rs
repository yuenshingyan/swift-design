...
//! English.
//!
//! There are seven artifact kinds. A demo session writes designs
...
//! canvas). A campaign session writes campaigns (ads on an IAB unit
//! canvas). Each kind has its own rule list.

use axum::routing::get;
use axum::{Json, Router};
use design_model::{
    Artwork, Campaign, Deck, Design, Document, Mailing, Print, QUESTIONS_PER_TURN_LIMIT, Social,
};
...
pub fn routes() -> Router<crate::AppState> {
    Router::new()
        ...
        .route("/schemas/campaign", get(get_campaign_schema))
        .route("/schemas/artwork", get(get_artwork_schema))
...
/// Returns the campaign JSON Schema, generated from the campaign
/// types.
async fn get_campaign_schema() -> Json<schemars::Schema> {
    Json(schemars::schema_for!(Campaign))
}

/// Returns the artwork JSON Schema, generated from the artwork types.
async fn get_artwork_schema() -> Json<schemars::Schema> {
    Json(schemars::schema_for!(Artwork))
}
...
fn steps() -> Vec<String> {
    vec![
        "GET /schemas/design, GET /schemas/deck, GET /schemas/document, GET /schemas/social, GET /schemas/print, GET /schemas/mailing, GET /schemas/campaign, GET /schemas/artwork, and GET /schemas/question-set. Read the JSON Schemas.".to_owned(),
        "GET /sessions/{id}. Read the request, the artifact_kind, the options, the state, the messages, the question sets, and the answers. The artifact_kind is `demo`, `deck`, `document`, `social`, `print`, `mailing`, `campaign`, or `artwork`. A demo session writes designs. A deck session writes decks. A document session writes documents. A social session writes socials. A print session writes prints. A mailing session writes mailings. A campaign session writes campaigns. An artwork session writes artworks. There is no brief: the request and the answers are the input.".to_owned(),
        ...
        "The app asks the user for these, not you: the artifact kind, how the colors read, the number of variations; for a demo how much to build, what kind of product it is, what state the screens show, and the canvases; for a deck the audience, the tone, the scenario, the number of slides, how much goes on a slide, how much it leans on data, and how different the candidates are; for a document the audience, the tone, how much it leans on data, what kind of document it is, the paper, how much goes on a page, the number of pages, and how different the candidates are; for a social the audience, the tone, how much it leans on data, the platform, the format, what the post is for, the number of frames, and how different the candidates are; for a print the audience, the tone, how much it leans on data, what kind of print piece it is, the paper size, the orientation, the number of sheets, and how different the candidates are; for a mailing the audience, the tone, how much it leans on data, what kind of email it is, the email format, the number of emails, and how different the candidates are; for a campaign the audience, the tone, how much it leans on data, what kind of ad it is, the ad size, the number of ads, and how different the candidates are; for an artwork the audience, the tone, how much it leans on data, what kind of cover it is, the cover size, the number of covers, and how different the candidates are. Never ask about them, and never ask about them in other words. Ask only what this request raises and that list does not cover, such as the features to show or the data on a screen. Ask nothing when the request and the source files already say enough. Read them from the session options and follow them. A demo with several platforms wants one design per canvas: write one file per canvas, each with that canvas in `viewport`.".to_owned(),
        ...
        "Campaign session: write the campaign as JSON that conforms to GET /schemas/campaign. The campaign has `ads` and `size`, and no `viewport`. Set `size` to the size in the session options: `medium_rectangle`, `leaderboard`, `half_page`, `skyscraper`, or `mobile_banner`. PUT it to /campaigns/{id}-candidate-1. Use the session id as the base. The browser shows the candidates.".to_owned(),
        "Artwork session: write the artwork as JSON that conforms to GET /schemas/artwork. The artwork has `covers` and `size`, and no `viewport`. Set `size` to the size in the session options: `thumbnail`, `banner`, `header`, `album`, or `book`. PUT it to /artworks/{id}-candidate-1. Use the session id as the base. The browser shows the candidates.".to_owned(),
        "A 422 response lists every problem in error.details. Fix each one. PUT again.".to_owned(),
        "When the user asks for a change in the chat while an artifact is open, edit that artifact and PUT it again. When no artifact is open, write new candidates. A message that pins two or more candidates and asks to combine parts of them is a merge: write one new candidate under the next free number, and take each part from the candidate the user names for it. A message with is_regenerate true names units like `[screen 2]`, `[slide 2]`, `[page 2]`, `[frame 2]`, `[sheet 2]`, `[email 2]`, `[ad 2]`, or `[cover 2]`: write those units anew, without their old markup, and keep the rest. POST /designs/{id}/fork, POST /decks/{id}/fork, POST /documents/{id}/fork, POST /socials/{id}/fork, POST /prints/{id}/fork, POST /mailings/{id}/fork, POST /campaigns/{id}/fork, or POST /artworks/{id}/fork copies a candidate to the next free number; the user presses Fork for that, you do not.".to_owned(),
        ...
        "After you save a social, look at it: GET /socials/{id}/frames/{n}.png returns a PNG of frame n, 1-based. Review every frame the same way and PUT the social again. GET /socials/{id}/export returns the social as one HTML file. GET /socials/{id}/export.pdf returns it as a PDF, one sheet per frame, the file a LinkedIn carousel takes. GET /socials/{id}/export.zip returns one PNG per frame in a zip, the files an Instagram carousel takes. Add ?frame=N to the HTML or PDF export to export only frame N.".to_owned(),
        "After you save a print, look at it: GET /prints/{id}/sheets/{n}.png returns a PNG of sheet n, 1-based. Review every sheet the same way and PUT the print again. GET /prints/{id}/export returns the print as one HTML file. GET /prints/{id}/export.pdf returns it as a PDF, one PDF page per sheet, the file a print shop takes. GET /prints/{id}/export.zip returns one PNG per sheet in a zip. Add ?sheet=N to the HTML or PDF export to export only sheet N.".to_owned(),
        "After you save a mailing, look at it: GET /mailings/{id}/emails/{n}.png returns a PNG of email n, 1-based. Review every email the same way and PUT the mailing again. GET /mailings/{id}/export returns the mailing as one HTML file. GET /mailings/{id}/export.pdf returns it as a PDF, one PDF page per email. GET /mailings/{id}/export.zip returns one PNG per email in a zip. GET /mailings/{id}/export.email.zip returns one email-client HTML file per email in a zip: inlined styles, a 600 px table shell, and Outlook support. Send that file from your email service. Add ?email=N to the HTML or PDF export to export only email N.".to_owned(),
        "After you save a campaign, look at it: GET /campaigns/{id}/ads/{n}.png returns a PNG of ad n, 1-based. Review every ad the same way and PUT the campaign again. GET /campaigns/{id}/export returns the campaign as one HTML file. GET /campaigns/{id}/export.pdf returns it as a PDF, one PDF page per ad. GET /campaigns/{id}/export.zip returns one PNG per ad in a zip: the files an ad platform takes. Add ?ad=N to the HTML or PDF export to export only ad N.".to_owned(),
        "After you save an artwork, look at it: GET /artworks/{id}/covers/{n}.png returns a PNG of cover n, 1-based. Review every cover the same way and PUT the artwork again. GET /artworks/{id}/export returns the artwork as one HTML file. GET /artworks/{id}/export.pdf returns it as a PDF, one PDF page per cover. GET /artworks/{id}/export.zip returns one PNG per cover in a zip: the files the platforms take. Add ?cover=N to the HTML or PDF export to export only cover N.".to_owned(),
        "When the design, the deck, the document, the social, the print, the mailing, the campaign, or the artwork is written, POST /sessions/{id}/complete, or exit with code 0. The session moves to reviewing.".to_owned(),
...
fn routes_map() -> serde_json::Value {
    let routes = [
        ...
        ("schema_campaign", "GET /schemas/campaign"),
        ("schema_artwork", "GET /schemas/artwork"),
        ...
        ("save_campaign", "PUT /campaigns/{id}"),
        ("check_campaign", "POST /campaigns/render"),
        ("render_campaign", "GET /campaigns/{id}/render"),
        ("ad_image", "GET /campaigns/{id}/ads/{n}.png"),
        ("export_campaign_html", "GET /campaigns/{id}/export"),
        ("export_campaign_pdf", "GET /campaigns/{id}/export.pdf"),
        ("export_campaign_zip", "GET /campaigns/{id}/export.zip"),
        ("fork_campaign", "POST /campaigns/{id}/fork"),
        ("save_artwork", "PUT /artworks/{id}"),
        ("check_artwork", "POST /artworks/render"),
        ("render_artwork", "GET /artworks/{id}/render"),
        ("cover_image", "GET /artworks/{id}/covers/{n}.png"),
        ("export_artwork_html", "GET /artworks/{id}/export"),
        ("export_artwork_pdf", "GET /artworks/{id}/export.pdf"),
        ("export_artwork_zip", "GET /artworks/{id}/export.zip"),
        ("fork_artwork", "POST /artworks/{id}/fork"),
...
fn instructions() -> serde_json::Value {
    ...
    let example_print: serde_json::Value =
        serde_json::from_str(include_str!("../../../fixtures/sample-print.json"))
            .unwrap_or_default();
    let example_campaign: serde_json::Value =
        serde_json::from_str(include_str!("../../../fixtures/sample-campaign.json"))
            .unwrap_or_default();
    let example_artwork: serde_json::Value =
        serde_json::from_str(include_str!("../../../fixtures/sample-artwork.json"))
            .unwrap_or_default();
    let example_mailing: serde_json::Value =
        serde_json::from_str(include_str!("../../../fixtures/sample-mailing.json"))
            .unwrap_or_default();
    serde_json::json!({
        "purpose": "Turn a request into HTML designs, decks, documents, social posts, print pieces, emails, display ads, or cover art: ask a few questions in the chat, write candidates, then edit them from the chat. Swift Design keeps the workflow state, validates, renders, and lets the user edit. Swift Design makes no LLM API calls.",
        "session": "The run works on one session. Its id is in the SWIFT_DESIGN_SESSION_ID environment variable. The mode is in SWIFT_DESIGN_RUN_MODE: generation. The artifact kind is in SWIFT_DESIGN_ARTIFACT_KIND: demo, deck, document, social, print, mailing, campaign, or artwork. GET /sessions/{id} returns the state, the artifact_kind, the options, the question sets, the answers, and the messages.",
        "kinds": {
            ...
            "campaign": "A display ad or a set of A/B variants on an IAB unit canvas: a product launch, a sale, brand awareness, an event, retargeting, or an app install. Written as a campaign with `ads` and `size`, and no `viewport`. Saved under /campaigns.",
            "artwork": "A piece of cover art or a set of A/B variants on a cover-art canvas: a video thumbnail, a channel banner, a profile header, a podcast or album cover, or a book cover. Written as an artwork with `covers` and `size`, and no `viewport`. Saved under /artworks.",
        },
        "steps": steps(),
        ...
        "artwork_rules": ARTWORK_RULES,
        "charts": {
            "rules": CHART_RULES,
            "example": CHART_EXAMPLE,
        },
        "conventions": {
            "canvas": "a design's `viewport` in px (default 1440 by 900); a deck's fixed 1920 by 1080; a document's `paper`: 794 by 1123 for a4, 816 by 1056 for letter; a social's `format`: 1080 by 1080 for square, 1080 by 1350 for portrait, 1080 by 1920 for story, 1200 by 630 for landscape; a print's `size`: 559 by 794 for a5, 794 by 1123 for a4, 1123 by 1587 for a3, 816 by 1056 for letter, 1056 by 1632 for tabloid, with `orientation: 'landscape'` swapping width and height; a mailing's `format`: 600 by 800 for short, 600 by 1200 for standard, 600 by 1800 for long; a campaign's `size`: 300 by 250 for medium_rectangle, 728 by 90 for leaderboard, 300 by 600 for half_page, 160 by 600 for skyscraper, 320 by 100 for mobile_banner; an artwork's `size`: 1280 by 720 for thumbnail, 2560 by 1440 for banner, 1500 by 500 for header, 3000 by 3000 for album, 1600 by 2560 for book. Use px units. The server scales the canvas to any frame.",
            ...
            "node_reference": "[screen N, node a/b/c <tag.class>: text] for a design; [slide N, node a/b/c <tag.class>: text] for a deck; [page N, node a/b/c <tag.class>: text] for a document; [frame N, node a/b/c <tag.class>: text] for a social; [sheet N, node a/b/c <tag.class>: text] for a print; [email N, node a/b/c <tag.class>: text] for a mailing; [ad N, node a/b/c <tag.class>: text] for a campaign; [cover N, node a/b/c <tag.class>: text] for an artwork",
        ...
        "payloads": {
            ...
            "PUT /campaigns/{id}": "the campaign JSON",
            "POST /campaigns/render": "the campaign JSON; returns the rendered HTML, or every validation error",
            "PUT /artworks/{id}": "the artwork JSON",
            "POST /artworks/render": "the artwork JSON; returns the rendered HTML, or every validation error",
        },
        "routes": routes_map(),
        ...
        "example_campaign": example_campaign,
        "example_artwork": example_artwork,
    })
}
...
mod tests {
    ...
    use crate::instructions::{
        ARTWORK_RULES, CAMPAIGN_RULES, CHART_EXAMPLE, DECK_RULES, DEMO_RULES, DOCUMENT_RULES,
        MAILING_RULES, PRINT_RULES, SOCIAL_RULES, instructions,
    };
    ...
    fn instructions_carry_social_rules_routes_and_the_example() {
        ...
        assert!(
            text.contains("demo, deck, document, social, print, mailing, campaign, or artwork")
        );
    ...
    #[test]
    fn instructions_carry_campaign_rules_routes_and_the_example() {
        let payload = instructions();
        assert_eq!(
            payload["campaign_rules"].as_array().unwrap().len(),
            CAMPAIGN_RULES.len()
        );
        let campaign = payload["campaign_rules"].to_string();
        assert!(campaign.contains("300 by 250 px"));
        assert!(campaign.contains("`size`"));
        assert!(campaign.contains("one message on each ad"));
        assert!(campaign.contains("safe margin"));
        assert!(campaign.contains("Link:"));
        assert!(campaign.contains("Do not write links between ads"));
        assert!(!campaign.contains("#screen-"));
        assert!(!campaign.contains("/decks/"));
        assert_eq!(
            payload["routes"]["schema_campaign"],
            "GET /schemas/campaign"
        );
        assert_eq!(payload["routes"]["save_campaign"], "PUT /campaigns/{id}");
        assert_eq!(
            payload["routes"]["ad_image"],
            "GET /campaigns/{id}/ads/{n}.png"
        );
        assert_eq!(
            payload["routes"]["export_campaign_zip"],
            "GET /campaigns/{id}/export.zip"
        );
        assert_eq!(
            payload["routes"]["fork_campaign"],
            "POST /campaigns/{id}/fork"
        );
        assert!(
            payload["kinds"]["campaign"]
                .as_str()
                .unwrap()
                .contains("`ads`")
        );
        let text = payload.to_string();
        assert!(text.contains("PUT it to /campaigns/{id}-candidate-1"));
        assert!(text.contains("`[ad 2]`"));
        let example: Campaign =
            serde_json::from_value(payload["example_campaign"].clone()).unwrap();
        assert_eq!(example.validate(), Vec::new());
        assert!(payload["example_campaign"].get("viewport").is_none());
    }

    #[test]
    fn instructions_carry_artwork_rules_routes_and_the_example() {
        let payload = instructions();
        assert_eq!(
            payload["artwork_rules"].as_array().unwrap().len(),
            ARTWORK_RULES.len()
        );
        let artwork = payload["artwork_rules"].to_string();
        assert!(artwork.contains("1280 by 720 px"));
        assert!(artwork.contains("`size`"));
        assert!(artwork.contains("one message on each cover"));
        assert!(artwork.contains("safe margin"));
        assert!(artwork.contains("Title:"));
        assert!(artwork.contains("Do not write a call to action button"));
        assert!(!artwork.contains("#screen-"));
        assert!(!artwork.contains("/decks/"));
        assert_eq!(payload["routes"]["schema_artwork"], "GET /schemas/artwork");
        assert_eq!(payload["routes"]["save_artwork"], "PUT /artworks/{id}");
        assert_eq!(
            payload["routes"]["cover_image"],
            "GET /artworks/{id}/covers/{n}.png"
        );
        assert_eq!(
            payload["routes"]["export_artwork_zip"],
            "GET /artworks/{id}/export.zip"
        );
        assert_eq!(
            payload["routes"]["fork_artwork"],
            "POST /artworks/{id}/fork"
        );
        assert!(
            payload["kinds"]["artwork"]
                .as_str()
                .unwrap()
                .contains("`covers`")
        );
        let text = payload.to_string();
        assert!(text.contains("PUT it to /artworks/{id}-candidate-1"));
        assert!(text.contains("`[cover 2]`"));
        let example: Artwork = serde_json::from_value(payload["example_artwork"].clone()).unwrap();
        assert_eq!(example.validate(), Vec::new());
        assert!(payload["example_artwork"].get("viewport").is_none());
    }
