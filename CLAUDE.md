...
- The HTML and PDF exports of the five multi-unit kinds take an optional one-based unit query (`?frame=N`, `?sheet=N`, `?email=N`, `?ad=N`, `?cover=N`) that exports that unit alone, named like the zip entry (`{id}-cover-2.pdf`). An out-of-range unit answers 404 with the valid range. The editors show a scope toggle (`All covers` / `Cover N`) beside the export links when a candidate has more than one unit; scoped, the PNG link downloads the unit's image and the mailing's Email link downloads that email's client HTML.
