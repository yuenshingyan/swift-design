...
- A demo carries no script, no form, and no `<button>`. Interaction is static: a link to `#screen-N` opens screen N (the frame ids are `screen-N`, counted from 1, and `NAVIGATION_SCRIPT` handles the click), `<details>` opens a menu, and `<input type='checkbox'>` or `radio` with `:checked` holds a toggle, a tab, or a modal. `markup.rs` rejects every other input type. The audit reports a box that looks like a control and does nothing as `static_control`; the polish loop feeds it back like a layout finding, and `deck_polish` drops it, since a slide is not clicked. The design editor has two modes (`PreviewMode`): Edit loads the editing script, so a click selects; Play loads none, so a click acts. A single-screen page posts `swift-design-navigate` with `target` when a screen link is clicked.
