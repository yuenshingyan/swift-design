...
- The HTML and PDF exports of the five multi-unit kinds take an optional one-based unit query (`?frame=N`, `?sheet=N`, `?email=N`, `?ad=N`, `?cover=N`) that exports that unit alone, named like the zip entry (`{id}-cover-2.pdf`). An out-of-range unit answers 404 with the valid range. The editors keep the export links whole-artifact and add a stateless unit menu beside them (`Cover N` with `Download as HTML/PDF/PNG`, plus email HTML for a mailing) when a candidate has more than one unit; a scope toggle was tried and removed because its sticky mode made later exports silently scoped.
