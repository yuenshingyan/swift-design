...
Swift Design is a harness that guides LLM agents to build HTML artifacts of six kinds:
...
Designs, decks, documents, socials, prints, and mailings are separate pipelines that share one workflow.
...
- A session has one `artifact_kind` (`demo`, `deck`, `document`, `social`, `print`, or `mailing`), set at creation. The engine, the chooser, and the studio read that one value. Do not infer the kind from ids or file contents.
...
- A deck page, a document page, a social page, a print page, and a mailing page use the same DOM vocabulary as a design page (`main.design`, `data-swift-design-*`), so the layout, navigation, editing, and audit scripts serve all six. Only the audience-follow script and the PPTX measurement script are deck-only.
...
- A demo carries no script, no form, and no `<button>`. Interaction is static: a link to `#screen-N` opens screen N (the frame ids are `screen-N`, counted from 1, and `NAVIGATION_SCRIPT` handles the click), `<details>` opens a menu, and `<input type='checkbox'>` or `radio` with `:checked` holds a toggle, a tab, or a modal. `markup.rs` rejects every other input type. The audit reports a box that looks like a control and does nothing as `static_control`; the polish loop feeds it back like a layout finding, and `deck_polish` drops it, since a slide is not clicked. The design editor has two modes (`PreviewMode`): Play, the default, loads no editing script, so a click acts; Edit loads it, so a click selects. The deck editor has the same pair as `DeckPreviewMode`, the document editor as `DocumentPreviewMode`, the social editor as `SocialPreviewMode`, the print editor as `PrintPreviewMode`, and the mailing editor as `MailingPreviewMode`: Read, the default, and Edit. A single-screen page posts `swift-design-navigate` with `target` when a screen link is clicked.
- An edit turn is focused or systemic (`edit_focus.rs`). A change whose references name screens, slides, pages, or frames sends only those units, each with its index, and the Chrome findings for them; a change that names none sends the whole artifact and every finding. The audit runs before every edit, at every effort. After the edit, `fix_edited_deck`, `fix_edited_document`, `fix_edited_social`, `fix_edited_print`, `fix_edited_mailing`, and `fix_edited_design` measure the touched units again and ask the model to fix what Chrome finds, for up to `polish_round_limit(effort)` rounds; the best version measured is saved.
...
- Design, deck, document, social, print, and mailing writes answer 409 unless the session is `generating` (or `reviewing`, for user saves from the editor). An external agent posts `/sessions/{id}/generate` before it writes.
- The app's answers live on `Session.options`, not in a question set: the artifact kind, the variation count (1 to `CANDIDATE_LIMIT`), the axes in `SHARED_AXES`, `DEMO_AXES`, `SPEECH_AXES`, `DECK_AXES`, `DOCUMENT_AXES`, `SOCIAL_AXES`, `PRINT_AXES`, and `MAILING_AXES`, the canvases for a demo (1 to `PLATFORM_LIMIT`), for a deck the scenario (`DECK_SCENARIOS`), the slide count, and the variety, for a document the page count (1 to `PAGE_COUNT_LIMIT`) and the variety, for a social the frame count (1 to `FRAME_COUNT_LIMIT`) and the variety, for a print the sheet count (1 to `SHEET_COUNT_LIMIT`) and the variety, and for a mailing the email count (1 to `EMAIL_COUNT_LIMIT`) and the variety. `RunOptions::axes` lists the picked axes for a kind, and `request_input` renders them; adding an axis means one entry in each table plus the `RunOptions` field and its two `match` arms (`axis`, `axis_slot_mut`), not a new branch. Every app question also takes an answer the user types: the chips are a shortcut, not the whole world. `option_problem` accepts a value from the fixed list or one that passes `is_custom_answer` (printable, at most `CUSTOM_ANSWER_LIMIT` characters), and `request_input` prints a typed answer as typed, because it has no label. The prompts tell the agent never to ask about them.
...
- Deleting a session deletes everything it owns: its designs, decks, documents, socials, prints, and mailings with their candidates, sidecars, and history (`DesignStore::delete_session`, `DeckStore::delete_session`, `DocumentStore::delete_session`, `SocialStore::delete_session`, `PrintStore::delete_session`, `MailingStore::delete_session`), and its uploads. A session id comes from the request text, so the same request makes the same id; without the sweep the next session would open on the deleted one's candidates. Deleting one artifact deletes its history too.
...
- Vocabulary: a design has screens; a deck has slides; a document has pages; a social has frames; a print has sheets; a mailing has emails. Use `artifact` only for code that serves every kind.
...
- Agents receive everything from the running app, never from repo files: instructions at `GET /instructions`, the schemas at `GET /schemas/{design,deck,document,social,print,mailing,question-set}`, the session at `GET /sessions/{id}`. Do not add agent-facing markdown or prompt files to the repo.
- The instructions payload lives in `crates/server/src/instructions.rs`. It carries one rule list per kind (`DEMO_RULES`, `DECK_RULES`, `DOCUMENT_RULES`, `SOCIAL_RULES`, `PRINT_RULES`, `MAILING_RULES`) and one example per kind. Update it whenever agent-visible behavior changes.
...
- Sample artifacts live in `fixtures/` (`sample-design.json`, `sample-deck.json`, `sample-document.json`, `sample-social.json`, `sample-print.json`, `sample-mailing.json`) for tests; the instructions payload embeds every example for agents.
...
- Routes: plural nouns, kebab-case: `/sessions`, `/sessions/{id}/messages`, `/designs`, `/decks`, `/documents`, `/socials`, `/prints`, `/mailings`, `/uploads`.
