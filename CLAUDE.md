...
- **Mailings**: emails and email sequences (newsletters, announcements, promotions, welcome emails, digests, invitations) on a 600 px wide canvas: short (600 by 800 px), standard (600 by 1200 px, the default), or long (600 by 1800 px). A mailing is a theme, a `format`, and `emails` with HTML and CSS. One email is a single send; two or more are a sequence, in send order. It has no transition and no controls. Exports: HTML, PDF (one PDF page per email), and a zip of one PNG per email (`export.zip`). No new export module: the PNGs come from the email screenshot path and the zip from the same `zip` crate.
...
- Mailing-only routes: `/mailings/{id}/emails/{n}.png`, `/mailings/{id}/export.pdf`, `/mailings/{id}/export.zip`.
