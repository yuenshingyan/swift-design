...
Swift Design is a harness that guides LLM agents to build HTML artifacts of eight kinds:
...
- **Prints**: posters, flyers, menus, programs, certificates, and signs on a paper-size canvas at 96 dpi: A5 (559 by 794 px), A4 (794 by 1123 px, the default), A3 (1123 by 1587 px), Letter (816 by 1056 px), or Tabloid (1056 by 1632 px), portrait or landscape. A print is a theme, a `size`, an `orientation`, and `sheets` with HTML and CSS. One sheet is a poster; two are the front and back of a flyer. It has no transition and no controls. Exports: HTML, PDF (one PDF page per sheet, the print-shop file), and a zip of one PNG per sheet (`export.zip`). No new export module: the PNGs come from the sheet screenshot path and the zip from the same `zip` crate.
- **Mailings**: emails and email sequences (newsletters, announcements, promotions, welcome emails, digests, invitations) on a 600 px wide canvas: short (600 by 800 px), standard (600 by 1200 px, the default), or long (600 by 1800 px). A mailing is a theme, a `format`, and `emails` with HTML and CSS. One email is a single send; two or more are a sequence, in send order. It has no transition and no controls. Exports: HTML, PDF (one PDF page per email), a zip of one PNG per email (`export.zip`), and a zip of one email-client HTML file per email (`export.email.zip`, from `email_html.rs`: inlined styles, resolved theme variables, a 600 px table shell, and Outlook ghost tables around the `columns` pattern). The PNGs come from the email screenshot path and the zips from the same `zip` crate.
- **Campaigns**: display ads and A/B variant sets (product launches, sales, brand awareness, events, retargeting, app installs) on a standard IAB unit canvas: medium rectangle (300 by 250 px, the default), leaderboard (728 by 90 px), half page (300 by 600 px), skyscraper (160 by 600 px), or mobile banner (320 by 100 px). A campaign is a theme, a `size`, and `ads` with HTML and CSS. One ad is a single placement; two or more are A/B variants of the same size, in priority order. It has no transition and no controls. Exports: HTML, PDF (one PDF page per ad), and a zip of one PNG per ad (`export.zip`, the files an ad platform takes). No new export module: the PNGs come from the ad screenshot path and the zip from the same `zip` crate.
- **Artworks**: cover art and A/B variant sets (video thumbnails, channel banners, profile headers, podcast and album covers, book covers) on a standard cover-art canvas: thumbnail (1280 by 720 px, the default), channel banner (2560 by 1440 px), profile header (1500 by 500 px), album cover (3000 by 3000 px), or book cover (1600 by 2560 px). An artwork is a theme, a `size`, and `covers` with HTML and CSS. One cover is a single piece; two or more are A/B variants of the same size, in priority order. It has no transition and no controls. Exports: HTML, PDF (one PDF page per cover), and a zip of one PNG per cover (`export.zip`, the files the platforms take). No new export module: the PNGs come from the cover screenshot path and the zip from the same `zip` crate.
...
- The app asks its own closed questions itself, on the same card, from fixed lists in `design-model` (`run_questions.rs`, `deck_questions.rs`): the color mode for every kind; the canvas, the scope, the product kind, the screen state, the fidelity, and the variations for a demo; the audience, the tone, and the evidence style for a deck, a document, a social, a print, and a mailing (`SPEECH_AXES`); the scenario, the length, the slide density, the candidates, and the variety for a deck; the document kind, the paper, the page density, the length in pages, the candidates, and the variety for a document; the platform, the format, the post goal, the length in frames, the candidates, and the variety for a social; the print kind, the print size, the orientation, the length in sheets, the candidates, and the variety for a print; the email kind, the email format, the length in emails, the candidates, and the variety for a mailing; the ad kind, the ad size, the length in ads, the candidates, and the variety for a campaign; the cover kind, the cover size, the length in covers, the candidates, and the variety for an artwork. These recur in every session, so hardcoding them keeps the wording and the options identical between runs. The agent asks only what the request itself raises. The setup planner turn also returns `suggestions`: an app axis the request answers by itself (`product_kind` from "a TODO app", `color_mode` from "dark"), as option key and fixed value. `RunOptions::suggest` fills the blank axes and lists them in `options.suggested`; the card shows them picked with a `suggested` tag, and a pick by the user drops the key.
...
Designs, decks, documents, socials, prints, mailings, campaigns, and artworks are separate pipelines that share one workflow.

- Separate per kind: the model types (`design.rs`, `screen.rs`; `deck.rs`, `slide.rs`; `document.rs`, `page.rs`; `social.rs`, `frame.rs`; `print.rs`, `sheet.rs`; `mailing.rs`, `email.rs`; `campaign.rs`, `ad.rs`; `artwork.rs`, `cover.rs`), the stores (`designs.rs`, `decks.rs`, `documents.rs`, `socials.rs`, `prints.rs`, `mailings.rs`, `campaigns.rs`, `artworks.rs`), the routes (`/designs/*`, `/decks/*`, `/documents/*`, `/socials/*`, `/prints/*`, `/mailings/*`, `/campaigns/*`, `/artworks/*`), the render entry points (`render.rs`, `deck_render.rs`, `document_render.rs`, `social_render.rs`, `print_render.rs`, `mailing_render.rs`, `campaign_render.rs`, `artwork_render.rs`), the patches (`patch.rs`, `deck_patch.rs`, `document_patch.rs`, `social_patch.rs`, `print_patch.rs`, `mailing_patch.rs`, `campaign_patch.rs`, `artwork_patch.rs`), the polish prompts (`polish.rs`, `deck_polish.rs`, `document_polish.rs`, `social_polish.rs`, `print_polish.rs`, `mailing_polish.rs`, `campaign_polish.rs`, `artwork_polish.rs`), the generation prompts (`generation.rs`, `deck_generation.rs`, `document_generation.rs`, `social_generation.rs`, `print_generation.rs`, `mailing_generation.rs`, `campaign_generation.rs`, `artwork_generation.rs`), and the editors (`editor.rs`, `deck_editor.rs`, `document_editor.rs`, `social_editor.rs`, `print_editor.rs`, `mailing_editor.rs`, `campaign_editor.rs`, `artwork_editor.rs`). Deck-only modules: `presenter.rs` and `pptx.rs`. Document-only module: `docx.rs`. Mailing-only module: `email_html.rs`.
...
- A print is the social pipeline with a paper size as the canvas: `PrintSize::viewport()` is always portrait, `Orientation::apply` swaps width and height for landscape, and `Print::viewport()` composes the two. The units are sheets (`[sheet N]`, `?sheet=N`, `sheets[N]`, `id="sheet-N"`); the noun does not collide with the shared DOM wrapper, so the render uses the plain document shape (`sheet-frame`). `Print::validate` caps the sheets at `SHEET_COUNT_LIMIT`, unlike the social's frame cap, which sits only at the option boundary. The polish loop drops `static_control`, and the rules tell the model that the first sheet is the face of the piece, text stays inside a safe margin because print shops trim, and `notes` carries print instructions such as stock or bleed.
- A mailing is the print pipeline with an email format as the canvas: `EmailFormat::viewport()` is always 600 px wide (`EmailFormat::ALL` in `mailing.rs`, the same shape as `PrintSize` without an orientation), and `Mailing::viewport()` reads it. The units are emails (`[email N]`, `?email=N`, `emails[N]`, `id="email-N"`); the noun does not collide with the shared DOM wrapper, so the render uses the plain document shape (`email-frame`). `Mailing::validate` caps the emails at `EMAIL_COUNT_LIMIT`, like the print's sheet cap. The polish loop drops `static_control` because a call to action is a styled `<a>` to an external target, and the rules tell the model that the layout is one column, the first email is the one the reader opens first, and `notes` carries the subject line and the preheader as a `Subject:` line and a `Preheader:` line.
- A campaign is the mailing pipeline with an IAB ad unit as the canvas: `AdSize::viewport()` names the five display units (`AdSize::ALL` in `campaign.rs`, the same shape as `EmailFormat`), and `Campaign::viewport()` reads it. Four of the five units sit below `MIN_VIEWPORT_SIDE`; that minimum binds designs only, so the campaign sizes test asserts the exact px values instead of `Viewport::is_valid`. The units are ads (`[ad N]`, `?ad=N`, `ads[N]`, `id="ad-N"`); the noun does not collide with the shared DOM wrapper, so the render uses the plain document shape (`ad-frame`). `Campaign::validate` caps the ads at `AD_COUNT_LIMIT`, like the mailing's email cap. The polish loop drops `static_control` because the call to action is a styled `<a>`, and the rules tell the model that each ad carries one message and one call to action, text stays inside a safe margin, the first ad is the primary variant, and `notes` carries the click-through URL and the alt text as a `Link:` line and an `Alt:` line.
- An artwork is the campaign pipeline with a cover-art unit as the canvas: `CoverSize::viewport()` names the five cover units (`CoverSize::ALL` in `artwork.rs`, the same shape as `AdSize`), and `Artwork::viewport()` reads it. Every cover size sits inside the valid viewport range, so the artwork sizes test asserts `Viewport::is_valid` and the exact px values. The units are covers (`[cover N]`, `?cover=N`, `covers[N]`, `id="cover-N"`); the noun does not collide with the shared DOM wrapper, so the render uses the plain document shape (`cover-frame`). `Artwork::validate` caps the covers at `COVER_COUNT_LIMIT`, like the campaign's ad cap. Every cover size is above Chrome's window minimum, so `screenshot_cover` needs no fixed-canvas pin, unlike `screenshot_ad`. The polish loop drops `static_control` because a button-like box on a thumbnail is deliberate furniture, and the rules tell the model that each cover carries one subject and one short line, type is sized for the render size (a thumbnail shows at about 168 px), text stays inside a safe margin (the banner's 1546 by 423 px safe area, the thumbnail's clear bottom right corner), the first cover is the primary variant, and `notes` carries the title context and the alt text as a `Title:` line and an `Alt:` line.
...
- A session has one `artifact_kind` (`demo`, `deck`, `document`, `social`, `print`, `mailing`, `campaign`, or `artwork`), set at creation. The engine, the chooser, and the studio read that one value. Do not infer the kind from ids or file contents.
...
- A deck page, a document page, a social page, a print page, a mailing page, a campaign page, and an artwork page use the same DOM vocabulary as a design page (`main.design`, `data-swift-design-*`), so the layout, navigation, editing, and audit scripts serve all eight. Only the audience-follow script and the PPTX measurement script are deck-only.
...
- A demo carries no script, no form, and no `<button>`. Interaction is static: a link to `#screen-N` opens screen N (the frame ids are `screen-N`, counted from 1, and `NAVIGATION_SCRIPT` handles the click), `<details>` opens a menu, and `<input type='checkbox'>` or `radio` with `:checked` holds a toggle, a tab, or a modal. `markup.rs` rejects every other input type. The audit reports a box that looks like a control and does nothing as `static_control`; the polish loop feeds it back like a layout finding, and `deck_polish` drops it, since a slide is not clicked. The design editor has two modes (`PreviewMode`): Play, the default, loads no editing script, so a click acts; Edit loads it, so a click selects. The deck editor has the same pair as `DeckPreviewMode`, the document editor as `DocumentPreviewMode`, the social editor as `SocialPreviewMode`, the print editor as `PrintPreviewMode`, the mailing editor as `MailingPreviewMode`, the campaign editor as `CampaignPreviewMode`, and the artwork editor as `ArtworkPreviewMode`: Read, the default, and Edit. A single-screen page posts `swift-design-navigate` with `target` when a screen link is clicked.
- An edit turn is focused or systemic (`edit_focus.rs`). A change whose references name screens, slides, pages, or frames sends only those units, each with its index, and the Chrome findings for them; a change that names none sends the whole artifact and every finding. The audit runs before every edit, at every effort. After the edit, `fix_edited_deck`, `fix_edited_document`, `fix_edited_social`, `fix_edited_print`, `fix_edited_mailing`, `fix_edited_campaign`, `fix_edited_artwork`, and `fix_edited_design` measure the touched units again and ask the model to fix what Chrome finds, for up to `polish_round_limit(effort)` rounds; the best version measured is saved.
...
- Design, deck, document, social, print, mailing, campaign, and artwork writes answer 409 unless the session is `generating` (or `reviewing`, for user saves from the editor). An external agent posts `/sessions/{id}/generate` before it writes.
- The app's answers live on `Session.options`, not in a question set: the artifact kind, the variation count (1 to `CANDIDATE_LIMIT`), the axes in `SHARED_AXES`, `DEMO_AXES`, `SPEECH_AXES`, `DECK_AXES`, `DOCUMENT_AXES`, `SOCIAL_AXES`, `PRINT_AXES`, `MAILING_AXES`, `CAMPAIGN_AXES`, and `ARTWORK_AXES`, the canvases for a demo (1 to `PLATFORM_LIMIT`), for a deck the scenario (`DECK_SCENARIOS`), the slide count, and the variety, for a document the page count (1 to `PAGE_COUNT_LIMIT`) and the variety, for a social the frame count (1 to `FRAME_COUNT_LIMIT`) and the variety, for a print the sheet count (1 to `SHEET_COUNT_LIMIT`) and the variety, for a mailing the email count (1 to `EMAIL_COUNT_LIMIT`) and the variety, for a campaign the ad count (1 to `AD_COUNT_LIMIT`) and the variety, and for an artwork the cover count (1 to `COVER_COUNT_LIMIT`) and the variety. `RunOptions::axes` lists the picked axes for a kind, and `request_input` renders them; adding an axis means one entry in each table plus the `RunOptions` field and its two `match` arms (`axis`, `axis_slot_mut`), not a new branch. Every app question also takes an answer the user types: the chips are a shortcut, not the whole world. `option_problem` accepts a value from the fixed list or one that passes `is_custom_answer` (printable, at most `CUSTOM_ANSWER_LIMIT` characters), and `request_input` prints a typed answer as typed, because it has no label. The prompts tell the agent never to ask about them.
...
- Deleting a session deletes everything it owns: its designs, decks, documents, socials, prints, mailings, campaigns, and artworks with their candidates, sidecars, and history (`DesignStore::delete_session`, `DeckStore::delete_session`, `DocumentStore::delete_session`, `SocialStore::delete_session`, `PrintStore::delete_session`, `MailingStore::delete_session`, `CampaignStore::delete_session`, `ArtworkStore::delete_session`), and its uploads. A session id comes from the request text, so the same request makes the same id; without the sweep the next session would open on the deleted one's candidates. Deleting one artifact deletes its history too.
...
- Candidate cards fix the height and let the width follow the canvas ratio: `frame_width_rem` in `canvas.rs` emits `--frame-width`, and the strip tiles do the same with `--tile-width`. A demo on a portrait canvas (`CanvasCard::is_bezelled`: the kind is `Demo` and `is_portrait_canvas`, the phone) gets a bezel; a document page and a social frame are portrait too but they are paper and a picture, so they get none. A narrow canvas (`is_narrow_canvas`, the tablet too) only changes the main preview height. Do not mix the two. The tab of a document canvas is named by its paper (`A4`, `Letter`); the tab of a social canvas by its format (`Square`, `Portrait`, `Story`, `Landscape`); the tab of a print canvas by its size and orientation (`A3`, `A4 landscape`); the tab of a mailing canvas by its format (`Email`, `Short email`, `Long email`); the tab of a campaign canvas by its ad size (`Medium rectangle`, `Leaderboard`); the tab of an artwork canvas by its cover size (`Thumbnail`, `Channel banner`). A print sheet is paper, a mailing email is a message, and an ad and a cover are pictures, so none gets a bezel, like a document page. A card's 13.5 rem stage-height floor is dropped when even one floored card cannot fit the row (`stage_height_rem`): a 728 by 90 leaderboard floored to 13.5 rem would be 109 rem wide.
...
- Vocabulary: a design has screens; a deck has slides; a document has pages; a social has frames; a print has sheets; a mailing has emails; a campaign has ads; an artwork has covers. Use `artifact` only for code that serves every kind.
...
- Agents receive everything from the running app, never from repo files: instructions at `GET /instructions`, the schemas at `GET /schemas/{design,deck,document,social,print,mailing,campaign,artwork,question-set}`, the session at `GET /sessions/{id}`. Do not add agent-facing markdown or prompt files to the repo.
- The instructions payload lives in `crates/server/src/instructions.rs`. It carries one rule list per kind (`DEMO_RULES`, `DECK_RULES`, `DOCUMENT_RULES`, `SOCIAL_RULES`, `PRINT_RULES`, `MAILING_RULES`, `CAMPAIGN_RULES`, `ARTWORK_RULES`) and one example per kind. Update it whenever agent-visible behavior changes.
...
- Sample artifacts live in `fixtures/` (`sample-design.json`, `sample-deck.json`, `sample-document.json`, `sample-social.json`, `sample-print.json`, `sample-mailing.json`, `sample-campaign.json`, `sample-artwork.json`) for tests; the instructions payload embeds every example for agents.
...
- Routes: plural nouns, kebab-case: `/sessions`, `/sessions/{id}/messages`, `/designs`, `/decks`, `/documents`, `/socials`, `/prints`, `/mailings`, `/campaigns`, `/artworks`, `/uploads`.
...
- Campaign-only routes: `/campaigns/{id}/ads/{n}.png`, `/campaigns/{id}/export.pdf`, `/campaigns/{id}/export.zip`.
- Artwork-only routes: `/artworks/{id}/covers/{n}.png`, `/artworks/{id}/export.pdf`, `/artworks/{id}/export.zip`.
- The HTML and PDF exports of the five multi-unit kinds take an optional one-based unit query (`?frame=N`, `?sheet=N`, `?email=N`, `?ad=N`, `?cover=N`) that exports that unit alone, named like the zip entry (`{id}-cover-2.pdf`). An out-of-range unit answers 404 with the valid range. The editors export through two stateless menu buttons: `All` downloads the whole artifact in one format (HTML, PDF, PNG zip, plus the mailing's email HTML zip), and `Cover N` (shown when a candidate has more than one unit) downloads the unit on screen. A scope toggle was tried and removed because its sticky mode made later exports silently scoped.
